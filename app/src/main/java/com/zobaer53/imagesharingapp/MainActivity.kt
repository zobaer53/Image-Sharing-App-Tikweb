...
class MainActivity : AppCompatActivity() {
    ...
    lateinit var imageListFromFirebase: ArrayList<MyModel>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val addPhotoButton = findViewById<MaterialButton>(R.id.addPhotoButton)
        ...
        mDatabaseReference!!.child("propertyFromDevice")
            .addListenerForSingleValueEvent(object : ValueEventListener {
                ...
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    for (snapshot in dataSnapshot.children) if (dataSnapshot.exists()) {
                        val item = snapshot.getValue(MyModel::class.java)
                        if (item != null) {
                            imageListFromFirebase.add(item)
                        }
                        Log.i(
                            "Tag",
                                    " Url1 = " + imageListFromFirebase
                                .toString()
                        )
                    }
